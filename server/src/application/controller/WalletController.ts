...
import { EtherAdapter } from '../../infrastructure/EtherAdapter';
...
class WalletController extends BaseController {
  ...
  async balance(req: Request, res: Response) {
    const address = req.query.address ?? Config.DEFAULT_WALLET_ADDRESS;
    const bal = await etherAdapter.queryAddressBalance(address as string);
  ...
  async transactionsReceived(req: Request, res: Response) {
    const address = req.query.address ?? Config.DEFAULT_WALLET_ADDRESS;
    const transactions = await etherAdapter.queryTransactionsReceivedAtAddress(address as string);
